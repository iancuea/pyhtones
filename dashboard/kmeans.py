...
def obtener_k_optimo(df_data, features, k_max=8):
    ...
    """
    # Clonamos para asegurar conversión limpia a números en el sub-proceso
    df_local = df_data[features].copy()
    for col in features:
        df_local[col] = pd.to_numeric(df_local[col], errors='coerce')
        
    X = df_local.dropna().values
    ...
    return max(2, min(k_optimo, len(X) // 2))
...
def clustering_carreras(df_data):
    ...
    if not features_disponibles:
        return {
            ...
            'figura_elbow': None, 'figura_clusters_2d': None, 'figura_clusters_3d': None,
            'tabla_clusters': None, 'metricas': {}
        }
    
    # === 2. PREPARACIÓN DE DATOS ===
    df_clean = df_data[[id_institucion, id_carrera] + features_disponibles].copy()
    
    # 💡 PÓLIZA DE SEGURO: Forzamos la conversión a numérico columna por columna.
    # Cualquier string residual o espacio rebelde se transformará en NaN de forma segura.
    for col in features_disponibles:
        df_clean[col] = pd.to_numeric(df_clean[col], errors='coerce')
    
    # Ahora sí, dropna eliminará los NaN reales y los strings que acabamos de desenmascarar
    df_clean = df_clean.dropna(subset=features_disponibles)
    
    if len(df_clean) < 3:
        return {
            'error': f'Datos insuficientes tras limpiar formatos: {len(df_clean)} registros con datos completos.',
            'figura_elbow': None, 'figura_clusters_2d': None, 'figura_clusters_3d': None,
            'tabla_clusters': None, 'metricas': {}
        }
    ...
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X) # <-- ¡Aquí ya va blindado y no explota! 🚀
    ...
    fig_elbow.add_trace(go.Scatter(
        ...
        line=dict(color='#5cb85c', width=3), marker=dict(size=8)
    ))
    
    fig_elbow.add_vline(
        x=k_optimo, line_dash='dash', line_color='#d9534f',
        annotation_text=f'K Óptimo = {k_optimo}', annotation_position='top right'
    )
    
    fig_elbow.update_layout(
        title='Elbow Method: Selección de Clusters Óptimos',
        xaxis_title='Número de Clusters (K)', yaxis_title='Inercia',
        height=350, width=590, margin=dict(l=60, r=20, t=60, b=50),
    ...
    df_plot_2d = pd.DataFrame({
        ...
        'Carrera': df_clean[id_carrera].values, 'Institución': df_clean[id_institucion].values
    })
    ...
    df_plot_3d = pd.DataFrame({
        'PC1': X_pca_3d[:, 0], 'PC2': X_pca_3d[:, 1], 'PC3': X_pca_3d[:, 2],
        'Cluster': [f'Cluster {c}' for c in clusters], 'Carrera': df_clean[id_carrera].values
    })
    
    colores_clusters = px.colors.qualitative.Set2[:k_optimo]
    color_map = {f'Cluster {i}': colores_clusters[i % len(colores_clusters)] for i in range(k_optimo)}
    
    fig_3d = px.scatter_3d(
        df_plot_3d, x='PC1', y='PC2', z='PC3', color='Cluster', hover_data=['Carrera'],
    ...
    metricas = {
        'K_optimo': k_optimo, 'n_carreras': len(df_clean),
    ...
    return {
        'error': None, 'figura_elbow': fig_elbow, 'figura_clusters_2d': fig_2d,
        'figura_clusters_3d': fig_3d, 'tabla_clusters': tabla_clusters, 'metricas': metricas
    }